...
from .mlp import MLP
from .normalization_layers import AdaIN, DimNorm, InstanceNorm
...
class DimFNOBlocks(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if norm is None:
            ...
            # self.norm = nn.ModuleList(
            #     [
            #         DimNorm(num_consts, out_channels)
            #         for _ in range(n_layers * self.n_norms)
            #     ]
            # )
            norms = [DimNorm(num_consts, out_channels) for _ in range(self.n_norms)] + \
                [
                    InstanceNorm(num_features=self.out_channels, n_dim=self.n_dim)
                    # getattr(nn, f"InstanceNorm{self.n_dim}d")(
                    #     num_features=self.out_channels
                    # )
                    for _ in range((n_layers-1)* self.n_norms)
                ]
        ...
        use_dim_norm = norm == "dim_norm"
        if use_dim_norm:
            self.forward = self.forward_with_preactivation_dim if self.preactivation else self.forward_with_postactivation_dim
        else:
            self.forward = self.forward_with_preactivation if self.preactivation else self.forward_with_postactivation
    ...
    def set_dimnorm_coeffs(self, weight_coeff, bias_coeff):
        for norm in self.norm:
            norm.set_coeffs(weight_coeff, bias_coeff)
    ...
    def forward_with_postactivation_dim(self, x, mu, std, index=0, output_shape=None):
        x_skip_fno = self.fno_skips[index](x)
        x_skip_fno = self.convs[index].transform(x_skip_fno, output_shape=output_shape)
        ...
        if self.norm is not None:
            x_fno = self.norm[self.n_norms * index](x_fno, std, mu)
        ...
        if self.mlp is not None:
            ...
            if self.norm is not None:
                x = self.norm[self.n_norms * index + 1](x, std, mu)
    ...
    def forward_with_preactivation_dim(self, x, mu, std, index=0, output_shape=None):
        # Apply non-linear activation (and norm)
        # before this block's convolution/forward pass:
        x = self.non_linearity(x)

        if self.norm is not None:
            x = self.norm[self.n_norms * index](x, std, mu)
